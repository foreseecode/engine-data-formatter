...
public class DataFormatter implements CommandLineRunner {


//    static final String WORKING_DIR = "C:\\Users\\dustin.benac\\formatter\\";
//    static final String CLIENT_KEY = "34134";
    static final String CLIENT_KEY = "3434215";
//    static final String MEASUREMENT_KEY = "5198417";
    static final String MEASUREMENT_KEY = "8840669";
    static final String EXPORT_JSON = "export.json";
    static final String OUTPUT_FILE = "formatted.json";
    static final String REQUEST_TYPE_SCORES = "1";
    static final String REQUEST_TYPE_IMPACTS = "2";

    public static void main(String[] args) throws Exception {
        SpringApplication.run(DataFormatter.class, args);
    }
    ...
    public void format() throws Exception {
        String requestType = REQUEST_TYPE_SCORES;
        String clientKey = CLIENT_KEY;
        String measurementKey = MEASUREMENT_KEY;
        String dataFile = EXPORT_JSON;
        String outputFile = OUTPUT_FILE;
        Console c = System.console();
        if (c != null) {
            String input;
            input = c.readLine("Request type (1=SCORES 2=IMPACTS): ");
            if (!Strings.isNullOrEmpty(input)) {
                requestType = input;
            }
            input = c.readLine("Client key (default is 3434215): ");
            if (!Strings.isNullOrEmpty(input)) {
                clientKey = input;
            }
            input = c.readLine("Measurement key (default is 8840669): ");
            if (!Strings.isNullOrEmpty(input)) {
                measurementKey = input;
            }
            input = c.readLine("Data file (default is export.json): ");
            if (!Strings.isNullOrEmpty(input)) {
                dataFile = input;
            }
            input = c.readLine("Output file (default is formatted.json): ");
            if (!Strings.isNullOrEmpty(input)) {
                outputFile = input;
            }
        }
